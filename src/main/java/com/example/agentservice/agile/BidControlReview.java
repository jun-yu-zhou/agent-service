...
public class BidControlReview {
    ...
    private static final String DASH_SCOPE_BASE_URL =
            "https://dashscope.aliyuncs.com/compatible-mode/v1";
    private static final String TEST_URL =
            "https://javawebemp.oss-cn-beijing.aliyuncs.com/%E4%B8%89%E5%B3%A1%E5%A4%A7%E5%AD%A6%E6%96%B0%E5%BB%BA%E5%AD%A6%E7%94%9F%E5%85%AC%E5%AF%93%E9%A1%B9%E7%9B%AE%E5%A4%9A%E6%B5%8B%E5%90%88%E4%B8%80%E7%AB%9E%E4%BA%89%E6%80%A7%E7%A3%8B%E5%95%86%E6%96%87%E4%BB%B69.17%E5%AE%9A%E7%A8%BF.doc";
    ...
    private static String reviewDocument(String documentUrl, JdkHttpTransport httpTransport) {
        ...
        ReActAgent agent = ReActAgent.builder()
                .name("bid-control-doc-review")
                .sysPrompt(DOC_REVIEW_PROMPT)
                .model(model)
                .build();
        Msg request = Msg.builder()
                .role(MsgRole.USER)
                .textContent("请完整阅读这份招标文件并执行控标风险审查。")
                .metadata(Map.of(
                        QwenDocDashScopeChatFormatter.DOC_URLS_METADATA_KEY, List.of(documentUrl),
    ...
    private static String summarize(String docReview, JdkHttpTransport httpTransport) {
        ...
        ReActAgent agent = ReActAgent.builder()
                .name("bid-control-report")
                .sysPrompt(REPORT_PROMPT)
                .model(model)
                .build();
        Msg request = Msg.builder()
                .role(MsgRole.USER)
                .textContent("以下是qwen-doc完成的审查草稿，请按系统要求生成最终报告：\n\n" + docReview)
                .build();
        Msg response = agent.call(request).block();
        if (response == null || response.getTextContent() == null
                || response.getTextContent().isBlank()) {

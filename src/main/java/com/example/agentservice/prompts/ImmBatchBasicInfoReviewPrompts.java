...
public final class ImmBatchBasicInfoReviewPrompts {
    ...
    private static final String COLLUSION_REVIEW_PROMPT_TEMPLATE = """
            ...
            判断时优先采用投标人自身信息（BIDDER_SELF）。附件合同、项目共用内容或其他背景信息可以帮助理解上下文，通常不宜单独作为投标人关联依据。
            ...
            LAYOUT和PAGE_NUMBER基于documentLayoutFacts审查跨投标人的页眉页脚、字体及字形、字号层级、段落、表格、章节、页码与分页特征；TEXT_SIMILARITY基于textSimilarityFacts审查非通用文本、共同错别字和非针对性编制。报价和分项报价关注可比口径下的异常一致性、规律性差异及其组合特征。少量分项的工程量、单价或总价相同可以作为线索，结合分项覆盖范围、项目共用清单可能性和其他关联证据判断其风险强弱，不宜自然推导为串通结论。共同履约地点、统一采购模板、单方残留、附件合同主体和正常竞争性降价通常需要其他证据相互印证后才具有风险意义。
    ...
    public static final String REPORT_PROMPT = """
            ...
            2. 报价、账户和主体信息以BIDDER_SELF为主要依据；附件合同、项目共用价格或条款更适合作为背景。报价的横向结论结合口径、参考价和首次/最终报价证据解释，避免把普通降价或项目共同信息写成异常。
            ...
            6. 用户给出的真实投标人名单决定第一、三表的投标人列；第三表保留全部名单，缺报价填“未发现明确报价”。第八表可根据已审查结论自行选择报价关联、文本与文件关联、主体信息关联、排版页码特征、综合风险等维度与行数，最后以“综合风险”收束；采用风险提示语气，不替代正式认定。

...
- `ImmService` / `ImmServiceImpl`：负责提交 OSS/IMM PDF 转图片任务并轮询获取任务结果，业务流程通过构造器注入使用。
...
PDF 转图片由 `PdfUtils` 编排，`PdfUtils` 注入 `ImmService`，返回包含文档名、页码和签名图片 URL 的 `ImmImagePage` 列表。不要在审查 Agent 中直接创建 OSS 或 IMM 客户端；新增流程应注入 `PdfUtils`。独立运行 `ImmBeforeReview` 时，main 会启动非 Web Spring 上下文并获取 `PdfUtils` Bean。
